...
if (onDashboard) {
  ...
  // Now actually login to test the full dashboard
  // Check if env has admin email
  let adminEmail = '';
  try {
    const env = readFileSync('./env.local.backup', 'utf8');
    const match = env.match(/ADMIN_EMAIL=(.+)/);
    if (match) adminEmail = match[1].trim();
  } catch {}
